...
def save_callback(sender, data):
    ...
    Civ_Info[Civ_Count] = [
    dpg.get_value("Civ_Name"), 
    dpg.get_value("Timescale"),  
    ...
    with dpg.table_row(parent="civ_table"):
        ...
        for i in range(0,Count):
            dpg.add_text(Civ_Info.values(i))
...
with dpg.window(label="Civ_Setup", width= 520, height= 720):
    with dpg.tab_bar():
        with dpg.tab(label="Civilization Creation"):
            ...
            with dpg.table(header_row=False, delay_search=True,
                           ...
                           borders_outerV=True, row_background=True , tag="civ_table") as civ_table:
                
                        dpg.add_table_column(label="Name") #Civilization Name
                        dpg.add_table_column(label="Timescale") #Timescale
                        dpg.add_table_column(label="Pop Growth Rate") #Population Growth Rate
                        dpg.add_table_column(label="Pop Death Rate") #Population Death Rate
                        dpg.add_table_column(label="Pop Area Density") #Population Area Density
                        dpg.add_table_column(label="Natural Disaster %") #Natural Disaster Chance
                        dpg.add_table_column(label="Max Natural Disasters") #Max amount of Natural Disasters
                        dpg.add_table_column(label="Plague %") #Chance of Plagues
                        dpg.add_table_column(label="Max Plagues") #Max Amounts of Plagues
                        dpg.add_table_column(label="Duration of Plagues") #Duration of Plagues in Years
                        dpg.add_table_column(label="Initial Population") #Initial Population
    ...
    #Ending for table
        with dpg.tab(label= "Chart Generation", tag="Chart_Tab"):
            dpg.add_text('Charts')
        with dpg.tab(label= "Exporting", tag="Export_Tab"):
